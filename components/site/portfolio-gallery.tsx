  ...
  return <>
    <div className="gallery">
      {items.map((image, index) => <button className="gallery-item" type="button" key={image.id} onClick={() => { setDirection("next"); setActive(index); }} aria-label={`${image.caption}: kattalashtirib ko'rish`}>
        <Image src={image.src} alt={image.alt} width={800} height={600} sizes="(max-width: 700px) 100vw, 33vw" />
    ...
    {item && <div className="image-viewer" role="dialog" aria-modal="true" aria-label={item.caption} onMouseDown={() => setActive(null)}>
      <div className="image-viewer-wrap" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="image-viewer-image"><button className="image-viewer-nav previous" type="button" onClick={() => navigate(-1)} aria-label="Oldingi rasm"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.5 5-7 7 7 7" /></svg></button>{leaving && <Image className={`image-viewer-photo leaving ${leaving.direction}`} src={leaving.item.src} alt="" width={1600} height={1200} sizes="100vw" aria-hidden="true" />}<Image key={item.id} className={`image-viewer-photo entering ${direction}`} src={item.src} alt={item.alt} width={1600} height={1200} sizes="100vw" priority /><button className="image-viewer-nav next" type="button" onClick={() => navigate(1)} aria-label="Keyingi rasm"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9.5 5 7 7-7 7" /></svg></button></div>
